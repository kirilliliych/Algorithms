...
std::vector<size_t> calculate_z_func(const std::string &str)
{
    size_t str_size = str.size();
...
size_t count_entries(std::string &pattern, std::string &text)
{
    size_t pattern_size = pattern.size();

    std::string pattern_text_concatenated = pattern + SEPARATOR + text;
    std::reverse(pattern.begin(), pattern.end());
    std::reverse(text.begin(), text.end());
    std::string reversed_pattern_text_concatenated = pattern + SEPARATOR + text;

    std::vector<size_t> prefixes = CalculatePrefixes(pattern_text_concatenated);
    std::vector<size_t> z_func_on_reversed = calculate_z_func(reversed_pattern_text_concatenated);
    size_t answer = 0;
    ...
    for (size_t i = pattern_size + 1; i < concatenated_str_size; ++i)
    {
        if ((prefixes[i] >= pattern_size) ||
            ((i - prefixes[i] >= pattern_size + 1) && 
             (z_func_on_reversed[pattern_size + 1 + (concatenated_str_size - 1 - (i - prefixes[i]))] + prefixes[i] >= pattern_size)))
        {
            ++answer;
        }
    }
...
int main()
{
    ...
    std::cout << count_entries(pattern, text);
