    ...
    void add_symb(char symb)
    {
        ...
        else
        {
            ...
            else
            {
                ...
                while ((cur_state_index != -1) && (states_[cur_state_index].to_[symb] == state_travelled_to_by_symb_index))
                {
...
//--------------required for current task---------------------
    ...
    int last_string_different_substrings_quantity = 0;
    int str_size = 0;

    int get_cur_string_different_substrings_quantity()      // should be called after each symbol added!
    {
        last_string_different_substrings_quantity += str_size - states_[states_[last_state_index].suf_link_].max_len_;
...
int main()
{
    ...
    {
        ac_val.add_symb(input[i]);
        int result = ac_val.get_cur_string_different_substrings_quantity(); 
